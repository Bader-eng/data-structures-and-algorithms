...
const getNames = (arr) => {
  let arrReverse=arr.map((value) =>{
    let arrstrung = value.name.split('');
    let revarr=arrstrung.reverse();
    let bader= revarr.join('');
    return bader;
  });
  return arrReverse;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let newarr=[];
  input.forEach(val=>{
    arr=val.filter(val => val%5===0 && typeof val ==='number');
    arr.forEach((val,i) => arr[i]=Math.pow(2,val));
    newarr.push(arr);
  });
  return newarr;
};
